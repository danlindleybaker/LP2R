 ...
 *
 * \param[in] tk : discrete time interval at which contribution from the
 * integral is sought. \param[in] td : time at which mu(t) first goes to zero
 * \param[in] w : frequency
 * \param[out] rint1 : int( (a+x)/((a+x)^2 + b x^2) dx/sqrt(x), x=1..inf);
 * I2(a,b) in Eqn B6 in the manuscript \param[out] rint2 : int( sqrt(x)/((a+x)^2
 * + b x^2) dx, x=1..inf); I1(a,b) in Eqn B7 in the manuscript
 */

void symbint(const double tk, const double td, const double w, double &rint1,
             double &rint2) {
  double a = tk / td;
  double b = w * w * tk * tk;
  ...
  double t2 = atan(2.0 * rt2 * alpha * delta /
 ...
 */
void GStarSlow(const double w, double &gp, double &g2p, double &ep,
               double &e2p) {
  ...
  for (int k = 1; k < n; k++) { // loop over phi
    ...
    for (int m = 1; m < n; m++) { // loop over phi_ST
      ...
      if (Alpha == 1) {
        dphiST = phi_ST_ar[m - 1] - phi_ST_ar[m];
      } else {
        dphiST = pow(phi_ST_ar[m - 1], Alpha) - pow(phi_ST_ar[m], Alpha);
      }
      tm = t_ar[m];
      tkm = tk * tm / (tk + tm);
