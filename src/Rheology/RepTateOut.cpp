...
void RepTateOpen(std::fstream &fRh, std::fstream &fDi) {
  ...
  fRh << "T=" << reptate_temp << ";  "
      ...
      << "PDI=" << Sys_PDI;
  if (has_chem) {
    fRh << ";  chem=" << reptate_chem;
  }
  if (has_label) {
    fRh << "; label=" << reptate_label;
  }
  if (has_origin) {
    fRh << "; origin=" << reptate_origin;
  }
  fRh << std::endl;
  ...
  if (CalcDielectric) {
    ...
    fDi << "T=" << reptate_temp << ";  "
        ...
        << "PDI=" << Sys_PDI;
    if (has_chem) {
      fDi << ";  chem=" << reptate_chem;
    }
    if (has_label) {
      fDi << "; label=" << reptate_label;
    }
    if (has_origin) {
      fDi << "; origin=" << reptate_origin;
    }
    fDi << std::endl;
  }
}
...
void RepTateWrite(std::fstream &fRh, std::fstream &fDi,
                  const double (&res)[5]) {
  if (fRh) {
    fRh << res[0] << " " << res[1] " " << res[2] << std::endl;
  }
  if (CalcDielectric) {
    if (fDi) {
      fDi << res[0] << " " << res[3] << " " << res[4] << std::endl;
    }
  }
}
...
void CSVOpen(std::fstream &fRh) {
  ...
  fRh.open(MechRelSpecFNM, std::ios::out);
  if (Add_header) {
    fRh << "w" << CSVdelimiter << " G'(w)" << CSVdelimiter << " G\"(w)"
        << CSVdelimiter << " eta(w)";
    if (CalcDielectric) {
      fRh << CSVdelimiter << " e'(w)" << CSVdelimiter << " e\"(w)"
    }
    fRh << std::endl;
    fRh << "rad/s" << CSVdelimiter << " Pa" << CSVdelimiter << " Pa"
        << CSVdelimiter << " Pa-s";
    if (CalcDielectric) {
      fRh << CSVdelimiter << " " << CSVdelimiter << " "
    }
    fRh << std::endl;
  }
}
...
void CSVWrite(std::fstream &fRh, const double (&res)[5]) {
  if (fRh) {
    fRh << res[0] << CSVdelimiter << " " << res[1] << CSVdelimiter << " "
        << res[2] << CSVdelimiter << " "
        << sqrt(res[1] * res[1] + res[2] * res[2]) / res[0];
    if (CalcDielectric) {
