use crate::{Parameters, CLPoly, DataArrays};
...
pub mod try_reptate;
pub fn time_step(index: i32, parameters: &mut Parameters, lpoly: &mut Vec<CLPoly>, data_arrays: &mut DataArrays) -> i32{
    if index != 0 {
    ...
    for i in 0..parameters.number_of_polymers as usize {
        ...
        if lpoly[i].alive {
            if lpoly[i].z_chain * parameters.phi_st.powf(parameters.alpha) < parameters.disentanglement_switch {
