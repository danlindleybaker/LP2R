...
public:
  ...
  double wt =
      0.0; /**< Weight fraction associated with the polymer in the ensemble */
  double Z_chain =
      0.0;        /**< Number of entanglements in the chain. Z_chain=mass/M_e */
  double z = 0.0; /**< Escaped number of entanglements from either ends of the
                     chain at current time */

  bool alive = true; /**< Becomes false if the chain completely relaxes */
  bool relax_free_Rouse = false; /**< True for unentangled chains */

  bool rept_set = false;   /**< Becomes true once further relaxation is assigned
                              tobe via reptation */
  double tau_d_0 = 1.0e22; /**< Reptation time for the zeroth mode; initialized
                              to a large number */
  ...
  double t_FRouse; /**< Relaxation time by free Rouse relaxation (Z_chain^2)
                      tau_e */
};
...
public:
  ...
  double operator()(int Z1, int Z2) {
    ...
    if (Z1 <= 500) {
      ...
      s1 = psum[499] + intg_psum(500, Z1 - 1);
    }
    if (Z2 < 500) {
      s2 = psum[Z2];
    } else {
      s2 = psum[499] + intg_psum(500, Z2);
    }
    return s2 - s1;
  }
};
