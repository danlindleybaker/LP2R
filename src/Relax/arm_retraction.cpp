...
void arm_retraction(const int np, const int indx) {
  ...
  double CLFpref =
      ret_pref_0 + (ret_pref - ret_pref_0) /
  ...
  if (indx == 0) { // first call; z=0
    ...
    if (Alpha == 1) { // Dan Comment - let's make this pow() call conditional as well...
    dz = 0.50 * CLFpref * sqrt(cur_time / phi_eq) * Log_DtMult / (z0 * sqrt(Psi_rept));}
    else {
    ...
    if (dz > (0.50 * LPoly[np]->Z_chain -
      ...
      if (OutTermRelaxPathways) { // Treat them as disentanglement
        if (OutputFormat == "CSV") {
          f_trelax << np << CSVdelimiter << LPoly[np]->wt << CSVdelimiter
                   << LPoly[np]->mass << CSVdelimiter << cur_time * tau_e
                   << CSVdelimiter << "2" << CSVdelimiter << phi_ST
                   << CSVdelimiter << Psi_rept << std::endl;
        } else {
          f_trelax << np << " " << LPoly[np]->wt << " " << LPoly[np]->mass
                   << " " << cur_time * tau_e << " "
                   << "2"
                   << " " << phi_ST << " " << Psi_rept << std::endl;
        }
