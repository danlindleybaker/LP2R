...
ak[kind][j][iter]=(long double *)malloc((sizeof(long double))*(2*N+1)) ;
bk[kind][j][iter]=(long double *)malloc((sizeof(long double))*(2*N+1)) ;
            if ( ( ak[kind][j][iter]== 0) ||
                 ( bk[kind][j][iter]==0)) {
        ...
        else if ( kww_eps*T > kww_delta*fabsl(S) )
            return -2; // cancellation
        else if ( iter && fabsl(S-S_last) + kww_eps*T < kww_delta*fabsl(S) )
